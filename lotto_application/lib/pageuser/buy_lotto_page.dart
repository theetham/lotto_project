...
class BuyLottoPage extends StatefulWidget {
  ...
  const BuyLottoPage({super.key, required this.userId});
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      if (data['success']) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("ไม่สามารถซื้อเลขล็อตโต้ได้")));
      }
    } else {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("เกิดข้อผิดพลาดในการซื้อ")));
    }
  } catch (e) {
    print("Error: $e");
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("ไม่สามารถเชื่อมต่อกับ API ได้")));
  }
}
