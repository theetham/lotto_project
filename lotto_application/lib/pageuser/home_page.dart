...
class _HomePageState extends State<HomePage> {
  ...
  late List<Widget> _pages; // ประกาศ _pages ที่นี่เพื่อให้สามารถเข้าถึงได้ทั่วทั้งคลาส

  @override
  void initState() {
    super.initState();
    userId = widget.userId; // เก็บค่า userId
    fetchWalletAmount(); // ดึงข้อมูลยอดเงินเมื่อหน้าโหลด
    // กำหนด _pages หลังจากที่มี userId แล้ว
    _pages = [
      BuyLottoPage(userId: userId),  // ส่ง userId ไปที่ BuyLottoPage
  ...
  Future<void> fetchWalletAmount() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (jsonData['success']) {
          setState(() {
            walletAmount = jsonData['data']['balance'] ?? 0; // เก็บยอดเงินที่ดึงมา
            isLoading = false; // เปลี่ยนสถานะการโหลดเป็น false
          });
        } else {
          setState(() {
            errorMessage = jsonData['message'] ?? "ไม่พบข้อมูลสมาชิก"; // แสดงข้อความ error
            isLoading = false;
          });
        }
      } else {
      ...
      setState(() {
        errorMessage = "เกิดข้อผิดพลาดในการเชื่อมต่อ: $e"; // แสดงข้อผิดพลาดในการเชื่อมต่อ
        isLoading = false;
      });
    }
  }
      ...
      appBar: AppBar(
        ...
        actions: [
          Padding(
            ...
            child: isLoading
                ? const CircularProgressIndicator(color: Colors.white) // แสดงโปรเกรสอินดิเคเตอร์ขณะโหลดข้อมูล
                : Row(
                    children: [
                      Text(
