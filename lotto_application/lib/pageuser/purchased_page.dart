...
class _PurchasedPageState extends State<PurchasedPage> {
  List<String> purchasedLottoNumbers = []; // ลิสต์ที่เก็บหมายเลขล็อตโต้ที่ผู้ใช้ซื้อแล้ว
  ...
  try {
    final response = await http.get(
      Uri.parse('http://192.168.100.106:3000/api/lotto/buy_al?userId=${widget.userId}'), // ส่ง userId ผ่าน query parameter
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Padding(
        ...
        child: isLoading
            ? const Center(child: CircularProgressIndicator()) // แสดงวงกลมโหลดขณะโหลดข้อมูล
            : errorMessage != null
                ? Center(child: Text(errorMessage!, style: const TextStyle(color: Colors.red))) // ข้อความข้อผิดพลาด
                : purchasedLottoNumbers.isEmpty
                    ? Center(child: Text('ไม่มีหมายเลขที่ซื้อ, หรือเจ้าของคือ: ${widget.userId}')) // แสดง userId
                    : ListView.builder(
